...
import proxy4eresource.proxy.util.ProxyHttpBodyUtil;
...
public class HttpRelayingHandler extends SimpleChannelUpstreamHandler {
	...
	/**
	 * The current, most recent HTTP request we're processing. This changes as
	 * multiple requests come in on the same persistent HTTP 1.1 connection.
	 */
	...
	/**
	 * Creates a new {@link HttpRelayingHandler} with the specified
	 * connection to the browser.
	 *
	 * @param browserToProxyChannel
	 *	      The browser connection.
	 * @param channelGroup
	 *	      Keeps track of channels to close on shutdown.
	 * @param relayListener
	 *	      The relay listener.
	 * @param hostAndPort
	 *	      Host and port we're relaying to.
	 */
	public HttpRelayingHandler(final Channel browserToProxyChannel,
			final ChannelGroup channelGroup, final RelayListener relayListener,
	...
	public void messageReceived(final ChannelHandlerContext ctx,
		...
		if (!readingChunks) {
			...
			originalHttpResponse = ProxyUtils.copyMutableResponseFields(
					hr,
					new DefaultHttpResponse(hr.getProtocolVersion(), hr
							.getStatus()));
			...
			if (StringUtils.isNotBlank(te)
				...
				if (hr.getProtocolVersion() != HttpVersion.HTTP_1_1) {
					...
					newHttpResponse = ProxyUtils.copyMutableResponseFields(
							hr,
							new DefaultHttpResponse(HttpVersion.HTTP_1_1, hr
									.getStatus()));
					if (!newHttpResponse
							.containsHeader(HttpHeaders.Names.TRANSFER_ENCODING)) {
						log.debug("Adding chunked encoding header");
						newHttpResponse.addHeader(HttpHeaders.Names.TRANSFER_ENCODING,
			...
			log.debug("Before update response from vendor: " + newHttpResponse.getHeaders().toString());
			ProxyHeaderUtil.getInstance().updateResponseFromVendorToBrowser(newHttpResponse,hostAndPort);
			log.debug("After update: {} | {} | " +	newHttpResponse.getHeaders().toString(), newHttpResponse.getStatus(), hostAndPort);
			if(Configure.getInstance().needRewriteUrl()){
				ProxyHttpBodyUtil.getInstance().updateBodys(newHttpResponse,hostAndPort);
			}
			...
			messageToWrite = this.httpFilter.filterResponse(
					this.currentHttpRequest, newHttpResponse);
		} else {
		...
		if (browserToProxyChannel.isConnected()) {
			...
			final boolean closeRemote = shouldCloseRemoteConnection(
					this.currentHttpRequest, originalHttpResponse,
					messageToWrite);
			final boolean closePending = shouldCloseBrowserConnection(
					this.currentHttpRequest, originalHttpResponse,
					messageToWrite);

			final boolean wroteFullResponse = wroteFullResponse(
			...
			ChannelFuture future = this.browserToProxyChannel
					.write(new ProxyHttpResponse(this.currentHttpRequest,
							originalHttpResponse, messageToWrite));

			if (writeEndBuffer) {
				// See the comment on this flag variable above.
				future = browserToProxyChannel
			...
			if (wroteFullResponse) {
				...
				future.addListener(new ChannelFutureListener() {
					public void operationComplete(final ChannelFuture cf)
							throws Exception {
						relayListener.onRelayHttpResponse(
								browserToProxyChannel, hostAndPort,
								currentHttpRequest);
					}
				});
			}
	...
	protected boolean closeEndsResponseBody(final HttpResponse res) {
	...
	protected boolean wroteFullResponse(final HttpResponse res,
			final Object messageToWrite) {
	...
	protected boolean shouldCloseBrowserConnection(final HttpRequest req,
	 ...
	 */
	protected boolean shouldCloseRemoteConnection(final HttpRequest req,
			final HttpResponse res, final Object msg) {
		if (res.isChunked()) {
			// If the response is chunked, we want to return unless it's
			// the last chunk. If it is the last chunk, then we want to pass
	...
	public void channelClosed(final ChannelHandlerContext ctx,
		...
		this.relayListener.onRelayChannelClose(browserToProxyChannel,
				this.hostAndPort, unansweredRequests,
				this.closeEndsResponseBody);
	}
	...
	protected final Queue<HttpRequest> requestQueue = new LinkedList<HttpRequest>();
